      ...
      local classCov = {
        ...
        functions = {
          ...
          details = utils.reduce(pack.class, function(result, currentClass)
            return not currentClass.method and result
              or utils.concat(
                result,
                utils.map(currentClass.method, function(method) -- result.concat
        ...
        branches = {
          ...
          details = not s.line and {} or utils.flatten(utils.map(
...
local parse = function(file_path)
  ...
  -- TODO: find out how to do this in the updated xml2lua...
  tree.options.noreduce = true
  local xml_parser = xml2lua.parser(tree)

  xml_parser:parse(xml_content)
  return unpackage(tree.root.report[1])
end
